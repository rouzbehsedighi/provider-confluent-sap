...
local.xpkg.deploy.configuration.%: local.xpkg.sync
	...
	@echo '{"apiVersion":"pkg.test.sap/v1","kind":"Configuration","metadata":{"name":"$*"},"spec":{"package":"$*-$(VERSION).gz","packagePullPolicy":"Never"}}' | $(KUBECTL) apply -f -
...
local.xpkg.deploy.provider.%: $(KIND) local.xpkg.sync
	...
	@echo '{"apiVersion":"pkg.test.sap/v1alpha1","kind":"ControllerConfig","metadata":{"name":"config-$*"},"spec":{"args":["-d"],"image":"$(BUILD_REGISTRY)/$*-$(ARCH)"}}' | $(KUBECTL) apply -f -
	@echo '{"apiVersion":"pkg.test.sap/v1","kind":"Provider","metadata":{"name":"$*"},"spec":{"package":"$*-$(VERSION).gz","skipDependencyResolution": $(XPKG_SKIP_DEP_RESOLUTION), "packagePullPolicy":"Never","controllerConfigRef":{"name":"config-$*"}}}' | $(KUBECTL) apply -f -
